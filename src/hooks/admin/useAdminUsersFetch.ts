...
export const useAdminUsersFetch = (): UseAdminUsersFetchReturn => {
  ...
  const fetchAdminUsers = useCallback(async (): Promise<AdminUser[]> => {
    ...
    try {
      ...
      if (!session) {
        console.log(
          'No authenticated session found, skipping admin users fetch'
        );
        setAdminUsers([]);
        return [];
      }
      ...
      if (!isDeveloperAdmin) {
        ...
        try {
          ...
          if (rpcError || !isDeveloperAdminRPC) {
            console.log(
              'User does not have admin permissions, skipping admin users fetch'
            );
            setAdminUsers([]);
            return [];
          }
          ...
          console.log(
            'RPC admin check failed, user likely does not have admin permissions'
          );
          setAdminUsers([]);
          return [];
        }
      ...
      const formattedAdminUsers = adminUsersData
