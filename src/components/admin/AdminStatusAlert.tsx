...
export function AdminStatusAlert({
  ...
  if (isDeveloper) {
    message =
      'You are confirmed as a developer with admin privileges by email verification.';

    if (isPermissionError) {
      icon = <Database className="h-4 w-4 text-[#811a2c]" />;
      message +=
        ' Some database permissions are still being applied and may take effect after refresh.';
    } else {
