...
export function ErrorAlert({ error }: ErrorAlertProps) {
  ...
  return (
    <Alert
      className={`mb-6 border-${isPermissionError ? '[#cbb26a]' : '[#ab233a]'} ${isPermissionError ? 'bg-amber-50' : 'bg-red-50'}`}
    >
      {isPermissionError ? (
      ...
      <AlertTitle className="text-[#811a2c]">
        {isPermissionError ? 'Database Access' : 'Error'}
      </AlertTitle>
