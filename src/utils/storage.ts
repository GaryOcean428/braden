...
export const ensureGuestAccess = async (bucketName: StorageBucketName) => {
  try {
    ...
    try {
      ...
      if (listError) {
        ...
        toast.error('Storage access error', {
          description:
            'Could not access storage buckets. Please sign in or contact support.',
        });
        return false;
      }
      ...
      try {
        ...
        if (error) {
          ...
          toast.error('Storage access error', {
            description:
              'Could not get permission to access storage. Please try signing in.',
          });
          return false;
        }
        ...
        toast.error('Storage access error', {
          description:
            'Storage access service unavailable. Please try again later.',
        });
        return false;
      }
    }
    ...
    toast.error('Storage access error', {
      description:
        'An unexpected error occurred while trying to access storage',
    });
    return false;
  }
};
...
export const getPublicUrl = (
  ...
  try {
    ...
    console.log(
      `Generated public URL for ${bucketName}/${path}:`,
      data.publicUrl
    );
